...
def extrair_sit2_features(
    ...
    """
    dev_by_id = {
        int(d.get("id") or d.get("user_id")): d 
        for d in db.get("developers", [])
    ...
    if nP == 0:
        ...
        # evita binarização; mantém gradiente de cobertura
        fullP = covP
...
def score_sit2(features: Dict, pesos: Dict) -> float:
    ...
    # MUST pode falhar no mundo real: penaliza no score, mas não zera como restrição dura
    #alinhado com o que mirko disse “é permitido uma equipe não cobrir todos os MUST… as equipes geradas terão um fit ruim”
    gargalo_score = score
...
def score_sit1_from_feats(
    ...
    #  ..penalize proporcionalmente, não só quando é 0
 #   if must_exists and covM == 0.0:
     #   _log(f"[SIT1.PENALTY] must_exists=True & covM=0 → y *= {nota_sem_must}")
      #  y *= float(nota_sem_must)
    if must_exists:
         # penaliza gradualmente: cobre 50% do MUST => score cai ~pela metade
        y *= max(float(nota_sem_must), float(covM))
...
def avaliar_dimension(
    ...
    nota_sem_must: float = 0.1, #mudei de 0.0 pra 0.1
) -> Dict:
    """
...
def avaliar_todos_as_dimensions(
    ...
    nota_sem_must: float = 0.1, #mudei de 0.0 p 0.1
) -> Dict:
    resultados = {}
